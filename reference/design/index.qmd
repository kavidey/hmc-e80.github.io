...
It's also possible to simplify this design by replacing the transistor switch with a [jumper](https://electronics.stackexchange.com/questions/74259/how-are-jumpers-used){target="_blank"} that you manually install before deploying.  Doing so will let you tune your gain to specific values by adding and removing jumpers, which doesn't require soldering. This is particularly handy if you need to trim your gain during deployment.  However, doing this presents other potential challenges: be sure to record your jumper configuration for each deployment, and make sure your jumpers aren't mechanically sensitive.  Extending this idea, you can simplify even further by replacing eliminating the $R_{f2}$ path and replacing $R_{f1}$ with a [potentiometer](http://www.resistorguide.com/potentiometer/){target="_blank"} (a variable resistor), but doing so poses significant deployment challenges since you need to measure your potentiometer value before each deployment.
...
You measure salinity by measuring salt water's conductivity, then backing out the salt concentration that would result in that conductivity.  Some online calculators can help with this process. [This](http://www.fivecreeks.org/monitor/sal.shtml){target="_blank"} converts from a measured conductivity to salinity, and [this](https://www.hamzasreef.com/Contents/Calculators/SalinityConversion.php){target="_blank"} converts from salinity to conductivity. Note that both converters depend on water temperature, so you'll probably need to deploy a temperature probe at the same time.  Here's an example of some [measurements](https://drive.google.com/file/d/1wtEeyfcI0ObH-54Jn9V4pzgGdI38T4-9/view?usp=sharing){target="_blank"} performed by E80 staff that show the temperature effects.
...
We stock a small number of [Pasco pH electrodes](https://www.pasco.com/prodCatalog/PS/PS-2102_pasport-ph-sensor/){target="_blank"} for pH measurements. These electrodes are usually used with interface boxes provided by Pasco (the Pasport system), but you can hook circuits directly to the BNC connector of the pH probe. 
...
This [application note](http://www.ti.com/lit/an/snoa529a/snoa529a.pdf){target="_blank"} has a ballpark estimate for the output impedance of a pH probe, which is very high (order 1 G$\Omega$). [This](http://www.explainthatstuff.com/how-ph-meters-work.html){target="_blank"} reference is a handy guide to the operation of pH sensors. The pH sensor guide explains that ions diffusing through glass, a slow process, provide the output current from a pH sensor, which explains why the output impedance for this sensor is so high.
...
Thermocouples generate very small voltages, so the Voltages must be amplified to be read by the Teensy. The easiest way to do the amplification is with an instrumentation amplifier. The one we have available is the Analog Devices AD623ANZ (data sheet [here](./assets/AD623.pdf){target="_blank"}).
...
**For TX**, we recommend using the [LM384](http://www.ti.com/lit/ds/symlink/lm384.pdf){target="_blank"} audio amplifier to drive the [DAEX25W-8](https://drive.google.com/open?id=1Jrxe618k8KY_t4pc5sp_lyUTn6-bCDne){target="_blank"} speaker.  The "Typical 5 W Amplifier" reference circuit in the LM384 datasheet is a good design for our purposes, though we can make a few small changes to improve it by omitting the volume control potentiometer and adding some bypass and coupling capacitors.  These changes are illustrated below:
...
**For Testing**, it's handy to be able to test your circuit from a function generator without getting all the Teensy code working.  The old function generators (the [HP/Agilent 33120A](https://www.keysight.com/us/en/assets/9018-04436/user-manuals/9018-04436.pdf){target="_blank"}) can help with that.  You can send a burst of sine waves, i.e.: a small number of sine wave cycles, by using internally triggered burst modulation on the [HP/Agilent 33120A](https://www.keysight.com/us/en/assets/9018-04436/user-manuals/9018-04436.pdf){target="_blank"}. This function is not available on the function generators built into the oscilloscope. 
...
You may refer to [this picture](https://drive.google.com/file/d/1oBBkm4o01ZZupTHBEN3McYiGxioi8_sz/view?usp=sharing){target="_blank"} of an example range finding setup. It is extremely important that your electronics and the tank water are not mixed, so [Use tape to hold electronics down](https://drive.google.com/file/d/1MLQDMKdA0Lh1eyxujIAGwgewM1vTXVeF/view?usp=sharing){target="_blank"} so they can't fall into the tank, and [keep bench top equipment as far from the tank edge as possible](https://drive.google.com/file/d/1MLQDMKdA0Lh1eyxujIAGwgewM1vTXVeF/view?usp=sharing){target="_blank"}. Always dry your test apparatus with a towel as you remove it from the water and be mindful of nearby electrical setups. 
:::

## AUV Turbidity Meter

This design adapts the lab turbidity meter to the requirements for being immersed and waterproofed. The instructions and procedure are [here](https://drive.google.com/open?id=1RYaInz-txbC4iaJD83_0HSwKYBlVGDN1HlEs4yDEMjM){target="_blank"}. A zip file of the design files is [here](https://drive.google.com/open?id=10QO_RUk4GNhHB0n_1xPkC-ebEDFHg0sX){target="_blank"}.


## Anemometer

An anemometer is used for measuring wind speed. It is useful for a surface vehicle, but not so much for a submersible. The instructions and procedures are [here](https://drive.google.com/open?id=1rY2bEjX3RP1EuHBjaoB50aiPz61I9q4W86KAzeNQdSo){target="_blank"}. A zip file of the design documents is [here](https://drive.google.com/open?id=10PVtvyJ87abJO67Chued8QdySKAWsk1L){target="_blank"}.
...
- The first is the MLX90316. The instructions and procedure are [here](https://drive.google.com/open?id=1f8EBwjwC41yVn9NPL-A11Ml1rJ3tqBN0ptcQc6E8O4U){target="_blank"}. 
- The second is the ACE128. The instructions and procedures are [here](https://drive.google.com/open?id=1nxSudsfMWcUmk4KY5xAjlgvL6IgpMP_TdypqefZIsCw){target="_blank"}. 
- For convenience, a zip file of the design files for both is [here](https://drive.google.com/open?id=10XoZwejIFrBTcVLfXJGrxdgBVczbB9Cn){target="_blank"}.  
...
The reference design for the velocity sensor was not terribly successful. Perhaps you can learn from other's mistakes. The instructions and procedures, such as they are, are [here](https://drive.google.com/open?id=1i0zIxPvECrV4dE9XiX8ruwt7mzBVitFCy8Mf-ID--Qk){target="_blank"}. A spreadsheet with the design calculations is [here](https://drive.google.com/open?id=1H4uxriuP9vmXTxEPW2_xlxATGKS0cJR1Ro92NOTeYJc){target="_blank"}. A spreadsheet with the experimental results is [here](https://drive.google.com/open?id=17crtsGbjBhGFo32qSEhczJHauxY05IONGZRKeeFRNcI){target="_blank"}. A zip archive with the design documents is [here](https://drive.google.com/open?id=10QkiZIVQhM2UwSnC5YuN2zTD1xlhsCxi){target="_blank"}.

Sufficiently ambitious teams might try using Doppler techniques with acoustic sensors to measure velocity.  This type of measurement is called a frequency modulated continuous wave (FMCW) measurement because (1) unlike time of flight measurements, you transmit a continuous sine wave, and (2) you vary the frequency of your wave during operation.  [This document](https://drive.google.com/file/d/0B7Ols9Km9Pa1Z205RDZ6SDczck0/view?usp=sharing){target="_blank"} describes the basics of FMCW processing.  You're on your own for making a triangle wave generator, a voltage controlled oscillator, and the receive mixers.

You can use recorded video of your robot, particularly if you have a reference scale in frame, to get a "true", ground-truth measurement of velocity. [VLC](http://www.videolan.org/vlc/index.html){target="_blank"} works well as capture software for a USB camera, and tripod-stabilized phones also record great video. We recommend [tracker](http://physlets.org/tracker/){target="_blank"} for tracking video tracking software. Some some students have used [video-physics](https://www.vernier.com/products/software/video-physics){target="_blank"}. Make sure a length reference like a meter stick is visible in the image that you capture. Also, attach a brightly colored piece of pool noodle to your robot so that it is visible above the water. This will enable you to use auto tracking features in the video analysis software. 
