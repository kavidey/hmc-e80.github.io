...
Submission Sheet: [html](./submission.qmd){target="_blank"} [docx](./submission.docx)

Prelab Help Sheet: None for this lab, but you've got this!  Review previous ones for ideas.

Writing Assignment: [html](./writing.qmd){target="_blank"}

Writing Template: [html](./writing-template.qmd){target="_blank"} [docx](./writing-template.docx)
...
This is a rotation lab, so not all teams will be working on this lab in the same week.  Even numbered teams (12, 24, 36, etc.) will do this lab in week 6, while Odd numbered teams (12, 24, 36, etc.) work on [Lab 5](/labs/lab5/index.qmd){target="_blank"}.  Odd numbered teams will do this lab in week 7, while Even numbered teams work on lab 5.  Be sure to watch the correct videos, do the correct quiz, and put in prelab effort for the correct lab!
...
* NASA Beginner's Guide to Aerodynamics (click [here](https://www.grc.nasa.gov/www/k-12/airplane/short.html){target="_blank"})
* E80 fluid measurements lecture notes (click [here](https://drive.google.com/file/d/1R1AHWD5GR1xRkoNJdMfdgkJvw1xmn8XO/view?usp=sharing){target="_blank"})
* Airfoil database (click [here](http://www.airfoiltools.com/search/index?m%5Bgrp%5D=naca4d&m%5Bsort%5D=1){target="_blank"})
* Airfoil and angle of attack definitions (click [here](http://www.esru.strath.ac.uk/EandE/Web_sites/11-12/MORE/hydrofoil/img/1.jpg){target="_blank"})
* NASA Wing geometry definitions (click [here](https://www.grc.nasa.gov/www/k-12/airplane/geom.html){target="_blank"}) 
* NASA Reynolds Number guide (click [here](https://www.grc.nasa.gov/www/BGH/reynolds.html){target="_blank"})
* Grandma Spark's step-by-step video guide to using COMSOL to model a cylinder in laminar flow (105 MB PowerPoint File, click [here](https://drive.google.com/drive/folders/1llecHQqT-IsAfxxNEL-jZWsQFFYYczDG?usp=sharing){target="_blank"}). Each slide on the PowerPoint contains a video!  Make sure that you press play on each video!
...
* COMSOL video tutorials for beginners (click [here](https://www.comsol.com/video/how-set-up-run-simulation-comsol-multiphysics){target="_blank"})
* How to use COMSOL 3D geometry tools (click [here](https://www.youtube.com/watch?v=1AggdrJzHyg){target="_blank"})
* How to form unions in COMSOL (click [here](https://www.youtube.com/watch?v=Q7fOawr58H0){target="_blank"})
* Computing lift and drag forces in COMSOL (click [here](https://www.comsol.com/blogs/how-do-i-compute-lift-and-drag/){target="_blank"})
:::
...
5. Verify that there is nothing but a Pitot tube (which looks like [this](https://dwyer-inst.com/series-160-stainless-steel-pitot-tube.html){target="_blank"}) and the force measurement stand in the wind tunnel. If there are other objects in the wind tunnel, remove them.
6. Observe the mounting of the Pitot tube in the wind tunnel. Ensure that the end of the tube is properly oriented directly facing the incoming airflow. Ensure that the pitot probe is connected to the [digital manometer](http://www.dwyer-inst.com/Product/Pressure/Manometers/Digital/Series475){target="_blank"}.
...
* this Excel spreadsheet [PitotVelocityCalculator](https://docs.google.com/spreadsheets/d/1JXKPa2uhD6FO39ZgqziKAnF7XNatieHe/edit?usp=share_link&ouid=105511935419108795487&rtpof=true&sd=true){target="_blank"}
* this [Google Sheet](https://docs.google.com/spreadsheets/d/1m22gxiGKOKdak-1UFEZurIgK5epST0CCiMBFQLzDiQM/copy?usp=sharing){target="_blank"},  
* the [Dwyer Air Velocity Calculator](https://dwyer-inst.com/en/flowcalc){target="_blank"} 
...
Second, simulate a full-scale model of your robot (found [here](https://drive.google.com/file/d/1S4ma0dsIwoG4uevQlVbE5IhP2XK05xuQ/view?usp=sharing){target="_blank"}) in water at various velocities such that the Reynolds number is comparable to the Reynolds number of the small-scale model of the robot in the wind tunnel.  
