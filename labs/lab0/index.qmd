...
Submission Sheet: Submit your [E72+E80 MATLAB Tutorial](./matlab-startup.qmd){target="_blank"} on the E80 Canvas page
...
2. Drill the mounting holes for your vertical motor. You will be mounting your AUV's motors in the same manner as the three motor assembly in [Practicum 1A Section 4.7](https://sites.google.com/g.hmc.edu/e79-practicum/module-1/practicum-1a){target="_blank"}. Take into account the 2'' difference in length when positioning holes.
3. Construct at least one additional motor from scratch. Reference [Practicum 1A Section 4.2](https://sites.google.com/g.hmc.edu/e79-practicum/module-1/practicum-1a){target="_blank"} for assembly instructions.  Each team will be provided with 2 used motors, but needs to make a third, and may make more to have backups.
...
Never charge above 12.6 V or allow your battery to discharge below 11.1V.
:::
...
1. [Simple LED Lesson](https://www.youtube.com/watch?v=wCJKMFGEuuI){target="_blank"}:
    ...
    where $V_S$ is the supply voltage, and $V_F$ and $I_F$ are the foward voltage and current of the diode respectively. $V_F$ and $I_F$ can be found on [LED datasheet](https://www.aopled.com/AOP_PDFs/L513SRD-C.pdf){target="_blank"}.

2. [Breadboard Internals](https://www.youtube.com/watch?v=S_a-qCy6Ekk){target="_blank"}.
3. [How to build a 555 Timer circuit](https://www.youtube.com/watch?v=Vbz_MKT7KnI){target="_blank"}. As you watch the video and build the circuit, identify how each connection you make is represented in the circuit diagram linked [here](https://www.elprocus.com/blinking-led-using-555-timer-ic/){target="_blank"}. Here is a [video on how the 555 works.](https://www.youtube.com/watch?time_continue=385&v=i0SNb__dkYI&feature=emb_logo){target="_blank"} 
...
You will be using an oscilloscope for most E80 labs. If you need a refresher on how to use an oscilloscope, refer to [Practicum 2B](https://sites.google.com/g.hmc.edu/e79-practicum/module-2/practicum-2b){target="_blank"} from E79.
...
3. One team member should solder header pins to the Teensy. **Teensys are in limited supply**, so practice soldering before this step. Use the reference images below as a guide, and make sure your pins are oriented as indicated in the references before you solder. 
...
Follow the [setup instructions](https://github.com/HMC-E80/E80?tab=readme-ov-file#how-to-use-this-repository){target="_blank"} in the README of the [E80 GitHub page](https://github.com/HMC-E80/E80-SP2023#git-setup){target="_blank"}.
...
3. Follow the instructions [here](https://docs.arduino.cc/software/ide-v2/tutorials/getting-started/ide-v2-uploading-a-sketch){target="_blank"} to compile and upload to the Teensy
...
5. Open [E80_Lab_01.ino](https://github.com/HMC-E80/E80-SP2023/blob/main/E80_Lab_01/E80_Lab_01.ino){target="_blank"}. 
6. Confirm that you can compile and upload the starter code to your Teensy.  Since your Teensy is not plugged into your motherboard here, this code will do little.  It is fully tested in [Lab 1](/labs/lab1/){target="_blank"}.

:::{.callout-note}
You must use a micro USB cable (see image [here](https://drive.google.com/file/d/0BwiB3SVueuOpd3VHSjZFczUwZ1E/view?usp=sharing&resourcekey=0-mdD6t8RsgB-2txW00XCDYw){target="_blank"}) to connect the Teensy to your computer before you can upload sketches.
:::
...
1. [Blinking lights on the Arduino board](https://www.pjrc.com/teensy/tutorial.html){target="_blank"}
2. [Hooking up an external LED](https://www.pjrc.com/teensy/tutorial2.html){target="_blank"}


## MATLAB Access

You will be using MATLAB to help analyze and interact with data throughout this course, so you need to install MATLAB.  You will first need to [Visit the MATLAB Portal](https://www.mathworks.com/academia/tah-portal/harvey-mudd-college-30622674.html){target="_blank"} and log in using your Mathworks account. If you do not have an account, create one using your `username@hmc.edu` credentials.

Then, do the following to install MATLAB on your computer:

1. [Visit the MATLAB Portal](https://www.mathworks.com/academia/tah-portal/harvey-mudd-college-30622674.html){target="_blank"}.
...
It is also possible to use an online copy of MATLAB [here](https://matlab.mathworks.com){target="_blank"}, but we discourage doing so in this course.
...
4. Plot one of the variables to check that data was read correctly (e.g., `plot(accelX)`)
...
If you have extra time during Lab 0 you may work on the MATLAB on-ramp tutorial.  You can find the assignment [here](./matlab-startup.qmd){target="_blank"}.
