...
3. One team member should 
    ...
    7. (Git savvy students will note that there are slicker ways to track both the E80 repository and your team's work, for instance by forking the E80 repository. We're doing this because it's easy and we expect to push few software updates. For more Git support, see the [GitHub Desktop documentation](https://docs.github.com/en/desktop))
...
4. Set the vertical motor control effort, `uV`, to be the product of the control gain, `Kp`, and the error, `depth_error`. The default value of Kp is 0, so you need to change it in `DepthControl.h`.  We recommend a Kp value somewhere between 100 - 1000, tuned based on how weak or strong your motor is.
...
2. While the sketch is running with the serial monitor open, insert the meter stick into the cylinder 
...
3. Calculate the error, `yaw_error`, as the difference between `yaw_des` and `yaw` (which you just calculated based on `state.heading` above). Since you're subtracting angles, use `angleDiff()`, a function we wrote at the top of `SurfaceControl.cpp`.
