...
format:
  html:
    other-links:
      - text: Breadboard Checklist
        href: ./checklist-breadboard.html
        icon: card-checklist
      - text: Soldered PCB Checklist
        href: ./checklist-soldered-pcb.html
        icon: card-checklist
      - text: Mechanical Integration Checklist
        href: ./checklist-integrate.html
        icon: card-checklist
---

After the proposal and jump start, your job is to build your robot. There are three major steps in a robot build:

1. Making a breadboard prototype ([checklist](./checklist-breadboard.qmd){target="_blank"})
2. Making a soldered PCB ([checklist](./checklist-soldered-pcb.qmd){target="_blank"})
3. Mechanically integrating your PCB and sensors into your robot frame. ([checklist](./checklist-integrate.qmd){target="_blank"})
 
You should aim to check in with a professor after each of these steps.
Checklists are provided for each step so that you can self assess your progress.
...
You may need to make penetrator bolts for this project stage. A method for applying marine epoxy to bolts that have been drilled out can be found [here](https://drive.google.com/file/d/0BwiB3SVueuOpc2JIeUl1Z3ktU2M/view?usp=sharing&resourcekey=0-3E2w3qqP6SJLheveTlKGJQ){target="_blank"}. Remember that marine epoxy needs to dry overnight, so making these bolts requires two days because epoxy is applied twice. Be sure epoxy always dries in a fume hood: staff can show you an appropriate hood to use. We use 1/4" holes for our penetrator bolts and we hold the undrilled bolts in vises while drilling out the central hole. We find that six wires is about the maximum that we can fit into one bolt before the epoxy starts failing. You will also need to drill holes for these penetrator bolts in your project box. Be sure to use 1/2" [step drill bits](https://www.amazon.com/Step-Drill-Bits/b?ie=UTF8&node=256287011){target="_blank"} (which we have in our cupboards) to drill out the holes. Standard drill bits will result in ragged holes that are not watertight.
...
You don't need to show your professors anything specific for this checkoff; we'll see your waterproofing in action in the [deployment](/project/field/){target="_blank"}.